...
Source: Step count data from Apple Health.
...
Step Count Categorization: Daily step counts will be grouped based on the periods above.
Outlier Analysis: Special analysis will focus on internship periods to identify deviations from holiday trends.
Weekday vs Weekend Comparison: Step counts during academic terms will be compared to measure the impact of Sabanci's academic program intensity.

Statistical Methods:
Group comparisons (e.g., summer vs academic terms).
Time series analysis to track step count changes over time.
T-tests to compare weekday and weekend step counts only during terms.

3. Visualization:

Create bar charts for average step counts in each period.
Use line graphs to show daily step count trends throughout the year.
Highlight unique cases (e.g., internships) in graphs with special markers or colors.
Clear the whole term (by extracting weekends) data to get pure data.
Expected Results:
Academic Terms: Step counts will be higher due to campus mobility.
Summer and Midterm Breaks: Step counts will decrease compared to academic terms.
Internship Periods: Step counts may increase during internships, differing from holiday trends.
Weekdays vs Weekends: Step counts will be higher on weekdays than weekends during academic terms.

Project Contributions:

This project will help explain how academic intensity and campus mobility affect physical activity at Sabancı University by clearing data from weekends and the terms other than academic term. 


Extra Analyses:
Seasonal differences: Study how step counts vary by season (summer, winter, fall, spring).
